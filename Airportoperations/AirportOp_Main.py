...
if setting == 'manual':
    ...
    pagelimit = [50,54]                   #Specify the amount of pages of data [-1] for last page
...
N_etvs_cat2_i = [1]   # Number of ETVs of category 2
start_delay_i = [10]  # Allowed start delay
...
for it1 in range(len(N_etvs_cat1_i)): 
    for it2 in range(len(N_etvs_cat2_i)): 
        for it3 in range(len(start_delay_i)): 
            ...
            T_charge_min = 15 #min charging time in minutes
            e_KE = 43.1 # MJ/Kg
            F_delay = 0.0001
            t_warmup = 5 #minutes warmuptime
            Xi_a =  [0.025, 0.05, 0.1, 0.2012, 0.25, 0.316, 0.3692, 0.62917, 0.7376, 0.8] 
            Xi_a = [x * 60 for x in Xi_a]
            Xi_a_warmup = [x * 0.5 for x in Xi_a]
            ...
            Kg_kerosene = ((model.objVal-F_delay*sum((variable_values['t'][a][len(P[a])-1]) for a in range(N_aircraft))))
...
if it1+it2+it3 >= 1:
    ...
    fig, ax = plt.subplots()	               
    plt.plot(taxi_delay, Kg_kerosene)    
